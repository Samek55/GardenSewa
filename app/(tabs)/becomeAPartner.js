            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                if (oversizedAssets.length > 0) {
                    Alert.alert(
                        ...
                        `${oversizedAssets.length} of your selected image(s) exceed the ${MAX_SIZE_MB} MB limit and were skipped.`,
                        [{ text: "OK" }]
                    );
                }

                if (validAssets.length > 0) {
                    setSelectedCompanyImages((prev) => [...prev, ...validAssets]);
                }
            } else {
    ...
    async function handleImageCertificatePick() {
        try {
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                if (validAssets.length > 0) {
                    setSelectedCertificates((prev) => [...prev, ...validAssets]);
                }
            } else {
                console.log("Selection canceled");
            }
        } catch (error) {
            console.log("Error picking certificate images:", error);
        }
    }
                            ...
                            <View style={styles.dropdownContainer}>
                                ...
                                <ScrollView style={styles.itemsList} nestedScrollEnabled={true}>
                                    {filteredAreaQuery?.length > 0 ? (
                                        filteredAreaQuery.map((areaName, index) => (
                                            <TouchableOpacity
                                                key={index}
                                        ...
                                        <View style={styles.noResultsContainer}>
                                            <Text style={styles.noResultsText}>
                                                {selectedArea ? "No area found" : "Please select a city first"}
                                            </Text>
                                        </View>
                                    )}
                                </ScrollView>
